...
def visualize_detections(image, boxes, classes, scores):
    for box, _cls, score in zip(boxes, classes, scores):
        ...
        x2 = int(x2)
        y2 = int(y2)
        if score > 0.6: # 예측 결과가 0.6보다 크다면
            cv2.rectangle(image, (x1, y1), (x2, y2), (0, 255, 0), 3) # 빨간박스
        else: # 예측결과가 0.6 보다 작다면
            cv2.rectangle(image, (x1, y1), (x2, y2), (0, 0, 255), 3) #  OpenCV 속성이기 때문에 BGR로 설정해야함 ( 초록 박스 )

        cv2.putText(image, text, (x1, y1), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 2, cv2.LINE_AA) # 이 부분 주석처리하면 image 속 물체에 박스만 쳐진상태로 출력됨
    return image
...
class RetinaNet:
    ...
    def predict(self, image):
        ...
        result = visualize_detections( # visualize_detections --> 네모박스, 사물이름을 이미지 위에 그려주는 함수
            image,
