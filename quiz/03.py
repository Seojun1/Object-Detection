    ...
    if label['name'] in file_names:
        ...
        for box in label['labels']:
            if 'box2d' in box:
                ...
                if not os.path.exists('../data/' + box['category']):
                    os.mkdir('../data/' + box['category'])

                # cv2.imwrite('../data/' + box['category'] + '/' + str(cnt) + '.jpg', crop_image)
                cv2.imwrite(f'../data/{box["category"]}/{str(cnt)}.jpg', crop_image)
                cnt += 1